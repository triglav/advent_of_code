...
fn check_xmas2(input: &[String]) -> usize {
    let width = input[0].len() as isize;
    let height = input.len() as isize;
    ...
    let check_xmas_detail = |x: isize, y: isize| -> bool {
        if !check_letter(x, y, 'A') {
            return false;
        }
        [(1, 1), (-1, -1), (1, -1), (-1, 1)]
            .into_iter()
            .filter(|c| check_letter(x - c.0, y - c.1, 'M') && check_letter(x + c.0, y + c.1, 'S'))
            .count()
            == 2
    };

    (0..height)
        .flat_map(|y| (0..width).filter(move |x| check_xmas_detail(*x, y)))
        .count()
}
